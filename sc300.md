...
#### What the renewal duration for requesting new password policy?
- 1 hour
...
- network connectivity between one dc & proxy service
...
#### What are the endpoints the Microsoft Entra Password Protection Proxy service (from the server) must have access to?
| Endpoint                                   | Purpose |
|---                                         |      ---|
| https://login.microsoftonline.com          | Authentication requests |
| https://enterpriseregistration.windows.net | Microsoft Entra Password Protection functionality |

## 📖 Day 30 (2025-08-18 05:56:22 AM <Monday> +05:30)

#### What are requirement for Microsoft Entra Password Protection DC agent?
- Windows Server 2012 and above
- No minimun forest or domain level required
- .Net 4.5
- AD _must_ user DFSR for sysvol replication

#### What are requirement for Microsoft Entra Password Protection Proxy Service?
- Windows server 2012 R2 and above
- .Net 4.7
- Allow domain controller to access the proxy service Server (via "Access this computer from the network" privilege assignment)
- Outbound TLS 1.2 HTTP Traffic
- _Security Administrator_ role/account is required to register the proxy server and forest with EntraID

#### Are Microsoft Entra Password Protection Proxy service and Application Proxy same?
- No

#### Can Microsoft Entra Password Protection Proxy service and Application Proxy be installed on same machine ? Why or Why not
- No

- **Reason :** Both services install different version of "Microsoft Connect Agent Updater service". These versions are incompatabile with each other.

#### What is the Entra role or access is requirest to register the Microsoft Entra Password Protection Proxy Service and forest with EntraID?
- Security Administrator

#### How Domain controllers can access the server hosting Microsoft Entra Password Protection Proxy Service?
- DC can get access via "_Access this computer from the network_" privilege assignment

#### Which has higher windows & .net version requirements - a) Microsoft Entra Password Protection DC agent _OR_ b) Microsoft Entra Password Protection Proxy Service ?
- b) Microsoft Entra Password Protection Proxy Service

#### What is the installer name for Microsoft Entra Password Protection Proxy Service?
- AzureADPasswordProtectionProxySetup.exe (AzureAD Password Protection Proxy Setup.exe)

#### What is the installer name for Microsoft Entra Password Protection DC agent?
- AzureADPasswordProtectionDCAgentSetup.msi (AzureAD Password Protection DC Agent Setup.msi)

#### Where does Microsoft Entra Password Protection Proxy Service generally installed ?
- Member server

#### How to automate the software installation for Microsoft Entra Password Protection DC agent?
- Following command can used for automating the installation:
- ```cmd
  msiexec.exe /i AzureADPasswordProtectionDCAgentSetup.msi /quiet /qn /norestart
  ```

#### Does Microsoft Entra Password Protection DC agent software installation requires reboot? Why or Why not?
- Yes

- **Reason :** Because password filter DLLs are only loaded or unloaded by a restart.

#### Can Microsoft Entra Password Protection DC agent be installed on non-DC machine?
- Yes (_but it would not be functional until the machine get promoted to DC_)

#### How to disable the auto upgrade (if required) for the Microsoft Entra Password Protection Proxy service?
- ```powershell
  Set-AzureADPasswordProtectionProxyConfiguration
  ```

#### How to get current configuration (like auto upgrade) of Microsoft Entra Password Protection Proxy service?
- ```powershell
  Get-AzureADPasswordProtectionProxyConfiguration
  ```

#### What cmdlet to use to find the installed version of Microsoft Entra Password Protection Proxy service?
- ```powershell
  Get-AzureADPasswordProtectionProxy
  ```

#### How to perform manual upgrade of Microsoft Entra Password Protection Proxy service?
- run the latest version of exe (download it from Microsoft download center)
- no need to un-install the current as it is in-place upgrade
- no reboot require
- can be automated using MSI procedures
- ```cmd
  AzureADPasswordProtectionProxySetup.exe /quiet
  ```

#### How to perform manual upgrade of Microsoft Entra Password Protection DC Agent?
- run the latest version of msi (download it from Microsoft download Center)
- no need to un-install the current as it is in-place upgrade
- no reboot require
- can be automated using MSI procedures
- ```cmd
...
#### What is Smart lockout in EntraID?
- Smart lockout is a security feature designed to protect user accounts from unauthorized access attempts, particularly from bad actors trying to guess passwords or using brute-force methods.
- It differentiates between legitimate user sign-ins and those from attackers.
- When a user fails to sign in multiple times, smart lockout temporarily locks the account, preventing further sign-in attempts for a specified duration. This helps ensure that genuine users can still access their accounts while blocking attackers from gaining access.

#### What is the default configuration for the first lockout?
-  default, smart lockout locks the account from sign-in attempts for one minute after 10 failed attempts.
-  (MS don't disclose the duration for subsequent lockout)

#### What happens if you typed the SAME wrong password multiples times?
- Smart lockout tracks the last three bad-password hashes to avoid incrementing the lockout counter for the same password.
- If someone enters the same bad password multiple times, this behavior _won't cause the account to lock out._

#### Is Smart lockout enabled by default in every Tenant?
- Yes

#### How to configure the behaviour of Smart lockout? (Or minimum license require to customize)
- Entra P1

#### What considerations EntraID takes to not lock genuine users?
...
#### Can Smart lockout be integrated with Onprem AD?
- Yes (if they use EntraID Connect)

#### What admins should consider while configuring pass-through authentication for Smart lockou?
- (LockoutThreshold)$_{EntraID\}$ < (LockoutThreshold)$_{WS-AD\}$

- (LockoutDuration)$_{EntraID\}$ > (LockoutDuration)$_{WS-AD\}$

#### Exercise - Manage Microsoft Entra smart lockout values

#### How to customize the Smart lockout values?
- **Pre-requisite:** EntraID P1 or higher

- _Follow the below steps to customize the smart lockout values:_
- Sign in to the Microsoft Entra admin center (https://www.entra.microsoft.com) using a Global administrator account.
- Open the portal menu and then select Protection.
- On the Protection menu, select Authentication Methods.
- On Authentications methods menu, select Password protection
- <img width="791" height="393" alt="image" src="https://github.com/user-attachments/assets/a92b9689-f323-49cf-923a-acea9680a4f1" />
...
#### How to authenticate with onprem application (kerberos) from EntraID account?
- Application proxy can be used to access onprem resources which requires Kerberos tickets for access.
- Application proxy uses Kerberos Constrained Delegation to support these application.

- more information :
- https://learn.microsoft.com/en-us/entra/identity/app-proxy/overview-what-is-app-proxy
- https://learn.microsoft.com/en-us/entra/identity/app-proxy/overview-what-is-app-proxy
 
#### Describe the Kerberos authentication flow in EntraID Application Proxy.
- <img width="838" height="274" alt="image" src="https://github.com/user-attachments/assets/6969b260-0f55-420f-a2b9-00a762e5c4d5" />
...
## Day 31 (2025-08-19 10:46:04 PM <Tuesday> +05:30)

#### Describe about EntraID Application Proxy?
...
#### What kind of authentication does onprem web application should use so Application Proxy can be used?
- Integrated Windows Authentication (IWA)

#### What are OSes EntraID support as Authentication platform in Azure?
- Windows Server 2019 Datacenter edition and later.
- Windows 10 1809 and later.
- Windows 11.
- Linux virtual machine

#### What is the benefit of using EntraID authentication for Azure VMs?
...
#### How to Configure Microsoft Entra sign-in for Windows VMs?
...
- <img width="835" height="223" alt="image" src="https://github.com/user-attachments/assets/9dc17c87-6df8-42d1-893f-190be4881b54" />

#### How to Configure Microsoft Entra sign-in for Linux VMs?
- While creating Linux VM
- On the Management tab, Check the box to enable Login with Microsoft Entra ID.
- Ensure System assigned managed identity is checked.
- Complete the Linux virtual machine setup

- **// todo : check how to integrate EntraID authentication post VM creation**


## 📖 Day 31 (2025-09-08 07:14:42 AM <Monday> +05:30)
...
- basic set of security setting provided by Microsoft to every customer.

#### What are the settings "Security Defaults" contains?
...
#### Where to find "Security Defaults" setting in EntraID portal.
- Goto Entra Admin Portal
- Identity --> Overview --> Properties --> Security Defaults --> Manage Security Defaults

#### Is Security Defaults require cost / license?
- No, it is free and available to everyone to provide a standard security settings to everyone.

#### Once Security Defaults is enabled, what is the grace period a user needs to register for MFA. What happen if they dont?
- Users must register for MFA within _14 days_.
- If they don't register, they wouldn't be able to login without registering for MFA.

#### Does Security Defaults block legacy authentication?
- Yes

#### Is Security Default enabled by default?
- Yes, if tenant is created after Oct 22, 2019.
...
#### Generally, when we disable Security Defaults?
- Mostly when organization implements Conditional Access policies.

#### What is Conditional Access (policies)?
- Microsoft Entra Conditional Access (CA) analyzes signals, such as user, device, and location, to automate decisions and enforce organizational access policies for resource.
- You can use CA policies to apply access controls like multifactor authentication (MFA).
- <img width="830" height="346" alt="image" src="https://github.com/user-attachments/assets/84379373-0a0a-46f0-a68c-07bacc0bd0ea" />

#### Describe CA policy components.
- **Assignments**
  - Users
  - Target Resources
  - Conditions
    ...
    - _Locations_
    - _Client Apps_
    - _Filter for devices_
- **Access Controls**
  - Grant
    - Block Access
    - Grant Access
      - _Require MFA_
      - _Require Device to be compliant_
      - _Require MS Entra hybrid joined device_
      - _Require password change_
  - Session
 
#### How CA Policy works or policy gets applied?
  - CA policies are **if-then** statements: If an assignment is met, then apply these access controls.
  - Here assigments can also be understood as conditions.
  - A user has to satify any one or all (based on configuration) conditions (users/target res/conditions), then access controls applied (allow/deny)
 
## 📖 Day 32 (2025-09-09 11:57:05 AM <Tuesday> +05:30)

#### What is Named Locations?
  - Named locations allows grouping of locations based on IP range or geographic location.
  - they can be defined as Trusted or Untrusted and used with Conditional Access Policies.

#### How to create Named Locations.
- Goto Entra Admin Portal
- Protection --> Conditional Access --> Manage Section --> Named Location
- New Location --> Provide a name
- Choose IP Ranges or Countries/Region
- Save.

#### What is the maximum number of Conditional Access Policies you can create in a tenant?
- Hard limit : 195
- Soft limit : 195 - 2 (or 3) MS Managed policies

#### What is Report-only Enforcement in Conditional Access?
- Before deploying any CA policies, we can use "Report-Only" as audit mode (like What-If) where we can see it is applying to correct set of user/groups with right conditions.
- Once we are sure, the output we can enable the policies.

#### What is license require for EntraID Conditional Access Policies?
- MS Entra P1
- MS Entra P2 (with Risky sign-in, Risky users, risk based sign-in options (from Identity Protection)
- M365 E3 (or up)

#### Where we can create Conditional Access policies in Portal?
- Goto Entra Admin portal
- Protection --> Conditional Access --> New Policy
- <img width="830" height="419" alt="image" src="https://github.com/user-attachments/assets/5661d1ef-ca45-4cf3-a2e1-8fdfd93e1436" />

#### What Entra Role(s) is required to work with Conditional Access policies?
- Global Admin
- Security Admin
- Conditional Access Admin

#### What kind of user accounts MS recommends to exclude from CA Policies?
- Emergency Accesso or Break glass
- Service account and service principal (like MS Entra Connect Sync Account)

#### Examples of creating conditional policies
- https://learn.microsoft.com/en-us/training/modules/plan-implement-administer-conditional-access/5-implement-conditional-access-policies-controls-assignments
